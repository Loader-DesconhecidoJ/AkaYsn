...
if syn then 
    ...
    executorInfo.hasWriteFile = true
elseif krnl then 
    executorInfo.name = "Krnl"
    executorInfo.hasWriteFile = true
elseif script_context then 
    executorInfo.name = "ScriptWare"
    executorInfo.hasWriteFile = true
elseif isfile then 
    executorInfo.name = "Unknown (com writefile)"
    executorInfo.hasWriteFile = true
end

print("🔧 Executor detectado: " .. executorInfo.name)

-- Se não tem writefile, avisa
if not executorInfo.hasWriteFile then
    print("⚠️ Seu executor não suporta writefile. Configurações não serão salvas.")
end
...
local function salvarStandId(userId)
    ...
    -- Método 1: writefile (se existir)
    if writefile then
        sucesso = pcall(function()
            local jsonData = game:GetService("HttpService"):JSONEncode(config)
            writefile("dio_stand_config.json", jsonData)
        end)
    end
    
    -- Método 2: Salvar em _G (fallback)
    if not sucesso then
        _G.DIO_SAVED_STAND = config
        sucesso = true
    end
    
    -- Método 3: Usar HttpService (se disponível)
    if not sucesso and game:GetService("HttpService") then
        pcall(function()
            game:GetService("HttpService"):JSONEncode(config) -- Só pra testar
        end)
    end
    ...
    if sucesso then
        ...
        warn("⚠️ Não foi possível salvar permanentemente")
    end
end
...
local function carregarStandId()
    ...
    -- Método 1: readfile
    if isfile and isfile("dio_stand_config.json") then
        config = pcall(function()
            local data = readfile("dio_stand_config.json")
            return game:GetService("HttpService"):JSONDecode(data)
        end)
    end
    
    -- Método 2: _G
    if not config and _G.DIO_SAVED_STAND then
        config = _G.DIO_SAVED_STAND
    end
    
    if config and config.lastUserId then
        print("📂 ID carregado: " .. tostring(config.lastUserId))
...
local function loadCustomAnim(animationId)
    -- PROTEÇÃO: verifica se o jogo TEM Animator
    if not hum then return nil end
    
    local animator = hum:FindFirstChild("Animator")
    if not animator then
        -- Cria um Animator se não existir
...
-- ========== DETECÇÃO DO TIPO DE JOGO ==========
local gameType = "Unknown"
local function detectGameCompatibility()
    ...
    if hasHumanoid and hasWorkspace then
        gameType = "Compatible"
        print("✅ Jogo compatível com sistema de combate")
    else
        gameType = "Incompatible"
        print("⚠️ Jogo pode não ser compatível com todas as funções")
    end
    
    return gameType == "Compatible"
end

-- Se for incompatível, entra em modo seguro
if not detectGameCompatibility() then
    print("🛡️ Ativando modo seguro (apenas visuais)")
    -- Desativa funções que podem dar erro
...
local function showSpeechBubble(imageId, side, duration, customHead)
    ...
    if not head then
        -- Procura a cabeça de várias formas
        if character then
    ...
    -- Se não tem head, não faz nada (evita erro)
    if not head or not head.Parent then 
        print("⚠️ Não foi possível mostrar balão: sem head")
        return 
    end

    -- Resto do código igual, mas com pcall
    pcall(function()
        local billboard = Instance.new("BillboardGui")
        billboard.Adornee = head
        billboard.Size = UDim2.new(3.5, 0, 3.5, 0)
        ...
        task.delay(duration, function()
            if billboard then
                ...
                task.delay(0.4, function() pcall(function() billboard:Destroy() end) end)
            end
        end)
    end)
end
...
local function getClosestTarget(maxDist)
    -- PROTEÇÃO: verifica se o jogo TEM inimigos
    if not workspace or not character then return nil end
    
    local closest, minDist = nil, maxDist
    local root = character:FindFirstChild("HumanoidRootPart")
    if not root then return nil end
    
    -- PROTEÇÃO: usa pcall para não crashar
    local success, result = pcall(function()
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("Humanoid") and obj ~= hum then
                if not currentStand or obj.Parent ~= currentStand then
    ...
    if not success then
        -- Se falhou, provavelmente não é jogo de luta
        return nil
    end
...
-- ========== MODO SAFE PARA QUALQUER JOGO ==========
local function activateSafeMode()
    print("🛡️ Ativando modo de compatibilidade máxima")
    
    -- Desativa funções agressivas
    SETTINGS.Enabled = false
    
    -- Impede que o script quebre o jogo
    local originalSetCustomAnims = setCustomAnims
    setCustomAnims = function()
        pcall(originalSetCustomAnims)
    end
    ...
    -- Override em funções problemáticas
    performM1 = function()
        print("⚠️ M1 desativado neste jogo (modo seguro)")
    end
    
    performKnifeThrow = function()
        print("⚠️ Facas desativadas neste jogo (modo seguro)")
    end
    
    startFinisher = function()
        print("⚠️ Finisher desativado (modo seguro)")
    end
end

-- Detecta automaticamente se precisa do modo seguro
task.wait(2) -- Espera o jogo carregar um pouco
if not player.Character or not player.Character:FindFirstChild("HumanoidRootPart") then
    print("⚠️ Personagem não detectado corretamente. Ativando modo seguro...")
    activateSafeMode()
end
